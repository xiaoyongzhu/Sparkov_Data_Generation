...
class Profile:
    ...
    def weight_to_cumsum(self, cat):
        ...
        # invert
        self.profile[cat] = {self.profile[cat]
                             [k]: k for k in self.profile[cat]}
    ...
    def prep_holidays(self):
        ...
        self.profile['date_wt']['time_of_year'] = self.weight_to_prop(
            days_nonleap)
        self.profile['date_wt']['time_of_year_leap'] = self.weight_to_prop(
            days)
    ...
    def sample_from(self, is_fraud):
        ...
        for i, num in enumerate(rand_date):
            ...
            chosen_cat = self.closest_rand(
                self.profile['categories_wt'], rand_cat[i])
            ...
            unix_time = datetime.strptime(str((chosen_date.strftime(
                '%Y-%m-%d') + ' ' + stamp)), '%Y-%m-%d %H:%M:%S').timetuple()
            epoch = str(calendar.timegm((unix_time)))
            # if str(chosen_cat) == inputCat:
            output.append('|'.join([str(trans_num), chosen_date.strftime(
                '%Y-%m-%d'), stamp, str(epoch), str(chosen_cat), str(chosen_amt), str(is_fraud)]))
            # else:
            #    pass
        return output, is_traveling, travel_max, fraud_dates

    def sample_from_online_purchase(self, is_fraud):
        fake = Faker()
        # randomly sample number of transactions
        num_trans = int((self.end - self.start).days *
                        np.random.random_integers(self.profile['avg_transactions_per_day']['min'],  # need normal, not uniform
                                                  self.profile['avg_transactions_per_day']['max']))
        num_trans = int(num_trans/3) # assuming the online purchase is around 1/3 of in-store purchase
